...
/*public void PrintGrid()
{
    ...
    for (int i = GRID_SIZE - 1; i >= 0; i--)
    {
        ...
        // Print vertical and diagonal connections
        if (i > 0)
        {
            for (int j = 0; j < GRID_SIZE; j++)
            {
                ...
                if (j < GRID_SIZE - 1)
                {
                    if (grid[i, j] != null && grid[i, j].GetExit("South-East") != null)
                    {
                        if (grid[i, j].IsExitLocked("South-East"))
                        {
                            string keyName = grid[i, j].GetExit("South-East").GetKey().ToString();
                            Console.Write(string.Format("{0,-11}", "    \\[" + keyName + "]  "));
                        }
                        else
                        {
                    ...
                    if (grid[i, j] != null && grid[i, j].GetExit("South-West") != null)
                    {
                        if (grid[i, j].IsExitLocked("South-West"))
                        {
                            string keyName = grid[i, j].GetExit("South-West").GetKey().ToString();
                            Console.Write(string.Format("{0,-11}", "    /[" + keyName + "]  "));
                        }
                        else
                        {
